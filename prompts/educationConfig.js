...
export const userSummaryLevel = (summaryLevel) => {
  if (summaryLevel === '비전공자') {
    return `비전공자 수준 요약은 다음과 같은 기준을 따르라.
[학습자 정의]
...
[요약 수준 예시]
탄소중립은 온실가스를 줄이거나 흡수해서 실제 배출량을 0으로 만드는 것이다. 이를 위해 교통, 에너지, 건물 등에서 다양한 정책이 시행되고 있다. 우리나라뿐 아니라 여러 나라가 기후변화를 막기 위해 노력하고 있다.`;
  }
  if (summaryLevel === '전공자') {
    return `전공자 수준 요약은 다음과 같은 기준을 따르라.
[학습자 정의]
...
[요약 수준 예시]
이 문서는 탄소중립을 단순한 온실가스 감축이 아니라, 에너지 체계·정책 구조·사회적 합의 메커니즘 전반의 전환을 요구하는 총체적 전략으로 제시한다. 특히 '에너지 정의(energy justice)'를 중심 축으로 삼아, 재생에너지 전환·시민 참여·탄소 가격제 등의 요소를 상호 연결하여 통합적 접근을 보여준다. 이러한 구조는 탄소중립을 단순한 환경 규제 차원이 아닌 사회 구조 개편의 프레임으로 재위치시키며, 기후 대응의 실효성과 정의 구현 간의 긴장 관계를 통찰한다.`;
  }
  return '';
};

export const userQuestionLevel = (questionLevel) => {
  if (questionLevel === '비전공자') {
    return (
...
export const fieldFeatures = (field) => {
  const map = {
    ...
    '철학':   '해당 문서의 핵심 내용을 바탕으로, 존재론적, 인식론적, 기술철학적 관점에서 그 의미와 본질을 성찰적으로 해석하라.',
